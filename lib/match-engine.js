...
export async function autoGenerateBracketsAndAssign({ tournamentId, userId }) {
  ...
  let hasMaleOpen = categories.some(c => c.gender === 'Male' && c.byWeight === false && c.eventType === 'Kumite');
  let hasFemaleOpen = categories.some(c => c.gender === 'Female' && c.byWeight === false && c.eventType === 'Kumite');

  const newCategoriesBatch = [];
  if (!hasMaleOpen) {
    const newCatRef = doc(collection(db, 'categories'));
    const newCat = {
      name: 'Male Open Kumite',
      tournamentId,
      tournamentName: tournament.name || '',
      eventType: 'Kumite',
      gender: 'Male',
