...
export default function TatamisPage() {
  ...
  return (
    <>
      <PageHeader
        title="Tatamis"
        description="Configure competition mats and assign referees. Live scoring · queue management · match assignment coming."
        actions={<Button onClick={openCreate} className="bg-primary hover:bg-primary/90"><Plus className="h-4 w-4 mr-2" /> New Tatami</Button>}
      />
      ...
      {loading ? <Card className="border-border/60"><CardContent className="p-10 text-center text-sm text-muted-foreground">Loading…</CardContent></Card>
        : filtered.length === 0 ? (
          <Card className="border-border/60"><CardContent className="p-16 text-center">
            ...
            <Button onClick={openCreate} className="bg-primary hover:bg-primary/90"><Plus className="h-4 w-4 mr-2" /> Create Tatami</Button>
          </CardContent></Card>
        ) : (
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{t.tournamentName || '—'}</p>
                        </div>
                      </div>
